...
fn main() {
    ...
    //
    // Skip in release: Tauri's own bundler places the binaries in
    // Contents/MacOS/ and owns that directory; trying to overwrite a
    // previously code-signed binary there causes "Permission denied".
    ...
    if profile == "debug" {
        ...
        for bin in &["exiftool", "mkvpropedit"] {
            ...
            if src.exists() {
                ...
                // Remove any existing copy first — macOS marks signed
                // binaries read-only, which would cause the copy to fail.
